package technology.sola.sketchy.game.rain;
...
import technology.sola.sketchy.game.chunk.Chunk;
import technology.sola.sketchy.game.chunk.TileComponent;
import technology.sola.sketchy.game.player.PlayerComponent;
    ...
    for (var view : world.createView().of(TileComponent.class, SpriteComponent.class, TransformComponent.class)) {
      ...
      if (wetness > THRESHOLD_EIGHT) {
        if (!spriteComponent.getSpriteId().equals(Constants.Assets.Sprites.ERASED)) {
          ...
          view.entity().addComponent(ColliderComponent.circle(Chunk.HALF_TILE_SIZE / 2f, Chunk.HALF_TILE_SIZE / 2f, Chunk.HALF_TILE_SIZE / 2f));
        }
      } else {
